...
class ProductPage(BasePage): 
    ...
    def added_selected_book(self):
        ...
        assert self.book_title == self.book_in_basket, f'Вы добавили в корзину {self.book_title}, а в корзине оказалась {self.book_in_basket}'

    
    def price_corresponds_to_the_book(self):

        self.сost_book_in_basket = self.browser.find_element(*PPL.MESSAGE_WITH_PRICE).text
        assert self.book_price == self.сost_book_in_basket, f'Стоимость выбранной книги {self.book_price}, цена книги в корзине {self.сost_book_in_basket}'
